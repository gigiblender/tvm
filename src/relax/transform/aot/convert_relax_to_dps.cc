...
/*!
 * \file relax/transform/aot/convert_relax_to_dps.cc
...
class ConvertRelaxMainToDPS : public ExprMutator {
 public:
  ...
  IRModule operator()() {
    ...
    if (func_return->IsInstance<TupleNode>()) {
      bool empty = runtime::Downcast<Tuple>(func_return)->fields.empty();
      if (empty) {
        VLOG(0) << "Function " << gv->name_hint << " is already in DPS or returns empty Tuple.";
        return mod_;
      }
    }
    ICHECK(func_return->IsInstance<VarNode>() || func_return->IsInstance<TupleNode>())
        << "Only support Var or Tuple returns for now but got " << func_return->checked_type();
    {
      ...
      if (const TupleNode* node = func_return.as<TupleNode>()) {
        auto tuple = runtime::GetRef<Tuple>(node);
    ...
    Function new_func =
        Function(new_params, new_body, TupleStructInfo({}, Span()), main_func->attrs);
 ...
 private:
  Expr VisitExpr_(const CallNode* op) override {
    if (op->op->IsInstance<ExternFuncNode>() || op->op->IsInstance<GlobalVarNode>()) {
      ...
      return Call(op->op, new_args, op->attrs, op->sinfo_args, op->span);
    }
  ...
  BindingBlock VisitBindingBlock(const BindingBlock& block) override {
    ...
    for (auto iter = block->bindings.rbegin(); iter != block->bindings.rend(); iter++) {
      ...
      if (const auto* var_binding = binding.as<VarBindingNode>()) {
        ...
        if (var_binding->value->IsInstance<TupleNode>() &&
            return_alias_.count(var_binding->var) > 0) {
          // Defining the returned tuple.
          auto tuple = runtime::Downcast<Tuple>(var_binding->value);
    ...
    for (auto iter = bindings_reverse.rbegin(); iter != bindings_reverse.rend(); iter++) {
      this->builder_->EmitNormalized(*iter);
    }
...
tvm::transform::Pass ConvertRelaxMainToDPS(Bool attach_io_to_attrs) {
  auto pass_func = [=](IRModule m, tvm::transform::PassContext ctx) {
    return relax::ConvertRelaxMainToDPS(m, attach_io_to_attrs->value != 0)();
  };
